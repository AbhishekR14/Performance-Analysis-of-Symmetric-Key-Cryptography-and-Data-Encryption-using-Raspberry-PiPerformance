...
noOfWordsInKey = 4  
def AESmain(plaintext, filename):
    
    key_data = "ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789"
    
    key_list = [random.choice(list(key_data)) for i in range(8)]
    key=''.join(key_list)
    ...
    en_data = open(os.path.join('encrypted_files', filename), 'wb')
    en_data.write(encryptedMessage)    
    en_data.close()
    return key
...
#AESmain(plaintext, 'abc.txt')
    #Base memory of AES is 21.6875 MB
